...
class TaxiRide(
  ...
  def this() {
    this(0, new DateTime(0), false, new GeoPoint(0.0, 0.0), 0, 0.0f, new GeoPoint(0.0,0.0))
  }
  ...
  override def toString: String = {
    ...
    sb.append((","))
    sb.append(startlocation.lon).append(",")
    sb.append(startlocation.lat)
    sb.toString()
  }
...
object TaxiRide {
  ...
  def fromString(line: String): TaxiRide = {
    ...
    try {
      ...
      val lonEnd = if (tokens(3).length > 0) tokens(3).toDouble else 0.0
      val latEnd = if (tokens(4).length > 0) tokens(4).toDouble else 0.0
      val lonStart = if (tokens.length >= 8) {if (tokens(7).length > 0) tokens(7).toDouble else 0.0} else 0.0
      val latStart = if (tokens.length >= 9) {if (tokens(8).length > 0) tokens(8).toDouble else 0.0} else 0.0
      val passengerCnt = tokens(5).toShort
      val travelDistance = if (tokens(6).length > 0) tokens(6).toFloat else 0.0f
      new TaxiRide(rideId, time, isStart, new GeoPoint(lonEnd, latEnd), passengerCnt, travelDistance, new GeoPoint(lonStart,latStart))
    }
    catch {
